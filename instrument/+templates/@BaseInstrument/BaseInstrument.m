classdef BaseInstrument < handle
    %BASEINSTRUMENT This is the base instrument class
    %   This class holds all properties and methods common to all
    %   instrument types. One specific example is 
    ...
    methods
        function this = BaseInstrument(instrName,instrManufacturer,...
                                            printOptions)
            %BASEINSTRUMENT Constructor for the BaseClass Instrument
            %   INPUTS: 
            %    INSTRMAN [char] - Name of the instrument manufacturer
            %    INSTRNAME[char] - Name of the instrument
            %    PRINTOPTIONS    - Structure specifying the print options
            %                      of the instrument.
            %   OUTPUTS: 
            
            %check if the user assigned printing options
            if not(exist('printOptions','var'))
                this.printOptions = ...
                    templates.BaseInstrument.DEFAULT_PRINT_OPTIONS;
            else
                this.printOptions = printOptions;
            end

            %assign the instrument name 
            this.name           = instrName; 
            this.manufacturer   = instrManufacturer;

            %attempt to autoregister the specified instrument upon build 
            this.autoRegDocs();

            %now display all found documentation 
            this.issueMessage(1,'Initializing Instrument: %s\n',this.name);
            this.issueMessage(1,'Manufacturer: %s', this.manufacturer);
            guideReport(this,2)
        end
