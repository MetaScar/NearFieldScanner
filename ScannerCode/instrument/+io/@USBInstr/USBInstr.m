classdef USBInstr < handle
    ...
    methods
        function this = USBInstr(Vendor,Address,varargin)
    ...
    methods(Access=protected)
        ...
        function          write(this,varargin)
            ...
            %if we are in the debug mode check for any errors 
            if this.COMM_DEBUG_MODE
                %print command to the terminal 
                fprintf('Sending: %s',sprintf(varargin{:}));
                %get the present error 
                err = this.getError; 
                errPresent = not(isempty(err)); 
                %if error is encounter print it to the terminal 
                if errPresent
                    warning('%s',err)
        ...
        function valOut = read(this,varargin)
            ...
            if this.COMM_DEBUG_MODE
                %print command to the terminal 
                fprintf('Querying: %s',sprintf(varargin{:}));
                %get the present error 
                err = this.getError; 
                errPresent = not(isempty(err)); 
                %if error is encounter print it to the terminal 
